# xxljob-enhance

> 为 XXL-Job 任务提供细粒度权限控制的增强系统

[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Node.js](https://img.shields.io/badge/node-%3E%3D20.0.0-brightgreen)](https://nodejs.org)
[![Docker](https://img.shields.io/badge/docker-%3E%3D20.10-blue)](https://www.docker.com)

[![Build and Push](https://github.com/YOUR_USERNAME/xxljob-enhance/actions/workflows/build.yml/badge.svg)](https://github.com/YOUR_USERNAME/xxljob-enhance/actions/workflows/build.yml)
[![PR Check](https://github.com/YOUR_USERNAME/xxljob-enhance/actions/workflows/pr-check.yml/badge.svg)](https://github.com/YOUR_USERNAME/xxljob-enhance/actions/workflows/pr-check.yml)
[![Release](https://github.com/YOUR_USERNAME/xxljob-enhance/actions/workflows/release.yml/badge.svg)](https://github.com/YOUR_USERNAME/xxljob-enhance/actions/workflows/release.yml)
[![Latest Release](https://img.shields.io/github/v/release/YOUR_USERNAME/xxljob-enhance)](https://github.com/YOUR_USERNAME/xxljob-enhance/releases/latest)

---

## 📖 项目简介

**xxljob-enhance** 是一个为 [XXL-Job](https://github.com/xuxueli/xxl-job) 提供**任务级权限管理**的增强系统。

XXL-Job 原生只支持**执行器级别**的权限控制，无法精确控制用户对单个任务的操作权限。本项目通过独立的权限管理系统，实现：

✅ **任务级权限控制** - 精确控制每个用户对每个任务的 `查看/执行/编辑` 权限
✅ **基于角色的访问控制 (RBAC)** - 灵活的角色和权限分配
✅ **权限 OR 合并逻辑** - 用户多角色权限自动合并
✅ **完整审计日志** - 记录所有任务操作历史
✅ **友好的前端界面** - 基于 React + Ant Design 的管理界面
✅ **一键 Docker 部署** - 开箱即用的容器化部署方案

---

## 🏗️ 系统架构

```
┌─────────────────────────────────────────────┐
│          xxljob-enhance Frontend            │
│       (React + Ant Design + Vite)           │
└─────────────────┬───────────────────────────┘
                  │ HTTP/REST API
┌─────────────────▼───────────────────────────┐
│          xxljob-enhance Backend             │
│       (NestJS + Prisma + SQLite)            │
│   ┌───────────────────────────────────┐     │
│   │  权限验证层 (RBAC)                │     │
│   │  - JobPermissionGuard            │     │
│   │  - AdminGuard                     │     │
│   │  - OR 逻辑权限合并                │     │
│   └───────────────┬───────────────────┘     │
│                   │                         │
│   ┌───────────────▼───────────────────┐     │
│   │  业务层 (Services)                │     │
│   │  - AuthService                    │     │
│   │  - PermissionsService             │     │
│   │  - XxlJobService (API Proxy)      │     │
│   └───────────────┬───────────────────┘     │
│                   │                         │
│   ┌───────────────▼───────────────────┐     │
│   │  数据层 (Prisma ORM)              │     │
│   │  - SQLite Database                │     │
│   └───────────────────────────────────┘     │
└─────────────────┬───────────────────────────┘
                  │ HTTP API
┌─────────────────▼───────────────────────────┐
│            XXL-Job Admin                    │
│       (原有的 XXL-Job 调度中心)              │
└─────────────────────────────────────────────┘
```

---

## 🚀 快速开始

### 前置要求

- **Docker** >= 20.10
- **Docker Compose** >= 2.0
- **XXL-Job Admin** (已部署并运行)

### 一键部署

1. **克隆项目**
   ```bash
   git clone https://github.com/your-org/xxljob-enhance.git
   cd xxljob-enhance
   ```

2. **配置环境变量**
   ```bash
   cp .env.example .env
   # 编辑 .env 文件，修改以下必需配置：
   # - JWT_SECRET (生产环境必改！)
   # - XXL_JOB_ADMIN_URL
   # - XXL_JOB_ADMIN_USERNAME
   # - XXL_JOB_ADMIN_PASSWORD
   ```

3. **启动服务**
   ```bash
   docker compose up -d
   ```

4. **访问应用**
   - 前端界面: http://localhost
   - API 文档: http://localhost:3000/api-docs
   - 后端 API: http://localhost:3000

5. **创建初始管理员账号**
   参见 [部署文档 - 初始化管理员账号](docs/deployment.md#初始化管理员账号)

---

## 📦 本地开发

### 后端开发

```bash
cd backend

# 安装依赖
npm install

# 配置环境变量
cp .env.example .env

# 生成 Prisma Client
npx prisma generate

# 运行数据库迁移
npx prisma migrate dev

# 启动开发服务器
npm run start:dev

# 运行测试
npm run test        # 单元测试
npm run test:e2e    # E2E 测试
npm run test:cov    # 测试覆盖率
```

### 前端开发

```bash
cd frontend

# 安装依赖
npm install

# 配置环境变量
cp .env.example .env

# 启动开发服务器
npm run dev

# 运行测试
npm run test

# 构建生产版本
npm run build
```

---

## 🔑 核心功能

### 1. 任务级权限控制

**三种权限类型**：
- **view** (查看) - 可以查看任务详情
- **execute** (执行) - 可以触发任务执行
- **edit** (编辑) - 可以启动/停止任务

**示例**：
- 开发者角色：可以查看和执行任务，但不能启动/停止
- 运维角色：拥有完整权限
- 测试角色：只能查看任务

### 2. OR 逻辑权限合并

用户可以拥有多个角色，权限会自动合并：

```
角色 A 对任务 1: canView=true, canExecute=false
角色 B 对任务 1: canView=false, canExecute=true
───────────────────────────────────────────────
合并结果:        canView=true, canExecute=true ✅
```

### 3. 审计日志

系统自动记录所有关键操作：
- 用户登录/登出
- 任务执行/启动/停止
- 权限变更
- 用户和角色管理

---

## 🛠️ 技术栈

### 后端
- **框架**: NestJS 11.x
- **ORM**: Prisma 6.x
- **数据库**: SQLite (可替换为 PostgreSQL/MySQL)
- **认证**: JWT (Access Token + Refresh Token)
- **API 文档**: Swagger/OpenAPI
- **测试**: Jest (88 个测试用例)

### 前端
- **框架**: React 19.x
- **UI 库**: Ant Design 5.x
- **路由**: React Router 7.x
- **构建工具**: Vite 7.x
- **状态管理**: React Context API
- **HTTP 客户端**: Axios
- **测试**: Vitest + React Testing Library

### DevOps
- **容器化**: Docker + Docker Compose
- **多阶段构建**: 优化镜像大小
- **健康检查**: 内置健康检查端点
- **数据持久化**: Docker Named Volumes

---

## 📚 文档

- [部署文档](docs/deployment.md) - Docker 部署、环境变量、初始化
- [API 文档](docs/api.md) - REST API 接口说明
- [Swagger UI](http://localhost:3000/api-docs) - 交互式 API 文档

---

## 🔒 安全性

- ✅ **JWT 认证** - Access Token (1h) + Refresh Token (7d)
- ✅ **bcrypt 密码加密** - 10 rounds salt
- ✅ **CORS 保护** - 可配置允许的来源
- ✅ **Helmet 安全头** - 防止常见 Web 攻击
- ✅ **限流保护** - 防止暴力破解 (15 分钟 100 次)
- ✅ **参数验证** - class-validator 自动验证
- ✅ **SQL 注入防护** - Prisma ORM 参数化查询
- ✅ **XSS 防护** - 前端输入清理

---

## 📊 测试覆盖率

| 模块 | 单元测试 | E2E 测试 | 前端测试 | 总计 |
|------|---------|---------|---------|------|
| 后端 | 42 | 37 | - | 79 |
| 前端 | - | - | 28 | 28 |
| **总计** | **42** | **37** | **28** | **88** |

运行所有测试：
```bash
# 后端测试
cd backend && npm run test && npm run test:e2e

# 前端测试
cd frontend && npm run test
```

---

## 🗂️ 项目结构

```
xxljob-enhance/
├── backend/                 # 后端 NestJS 应用
│   ├── src/
│   │   ├── auth/            # 认证模块 (JWT)
│   │   ├── users/           # 用户管理
│   │   ├── roles/           # 角色管理
│   │   ├── permissions/     # 权限服务
│   │   ├── jobs/            # 任务代理
│   │   ├── xxljob/          # XXL-Job API 集成
│   │   ├── audit/           # 审计日志
│   │   └── health/          # 健康检查
│   ├── prisma/              # Prisma Schema & 迁移
│   ├── test/                # E2E 测试
│   └── Dockerfile
├── frontend/                # 前端 React 应用
│   ├── src/
│   │   ├── components/      # UI 组件
│   │   ├── pages/           # 页面组件
│   │   ├── contexts/        # React Context (AuthContext)
│   │   ├── api/             # API 客户端
│   │   └── types/           # TypeScript 类型定义
│   ├── nginx.conf           # Nginx 配置
│   └── Dockerfile
├── docs/                    # 项目文档
│   ├── deployment.md        # 部署指南
│   ├── api.md               # API 文档
│   └── specs/               # 需求和设计文档
├── docker-compose.yml       # Docker Compose 配置
├── .env.example             # 环境变量示例
└── README.md                # 本文件
```

---

## 🤝 贡献指南

欢迎提交 Issue 和 Pull Request！

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/amazing-feature`)
3. 提交更改 (`git commit -m 'Add amazing feature'`)
4. 推送到分支 (`git push origin feature/amazing-feature`)
5. 提交 Pull Request

### 开发规范
- 遵循 ESLint 和 Prettier 配置
- 为新功能编写测试
- 更新相关文档

---

## 📝 待办事项 (Roadmap)

- [ ] 支持 PostgreSQL / MySQL 数据库
- [ ] 多租户支持
- [ ] 更细粒度的操作审计
- [ ] 导出审计日志为 CSV/Excel
- [ ] 支持 LDAP / OAuth2 单点登录
- [ ] 前端国际化 (i18n)
- [ ] 实时通知 (WebSocket)
- [ ] Grafana / Prometheus 监控集成

---

## ❓ 常见问题 (FAQ)

### Q: 为什么选择 SQLite 而不是 PostgreSQL？
A: SQLite 适合中小规模部署，无需额外的数据库服务。如需高并发场景，可以轻松切换到 PostgreSQL（只需修改 Prisma 配置）。

### Q: 如何迁移到 PostgreSQL？
A: 修改 `backend/prisma/schema.prisma` 中的 `datasource`，然后运行 `npx prisma migrate dev`。详见 [Prisma 文档](https://www.prisma.io/docs/concepts/database-connectors/postgresql)。

### Q: 系统支持多少用户？
A: 在默认配置下，可支持数百并发用户。如需更高性能，建议切换到 PostgreSQL 并启用负载均衡。

### Q: 可以脱离 XXL-Job 使用吗？
A: 不可以。本系统是 XXL-Job 的增强层，依赖 XXL-Job Admin API 获取任务信息。

### Q: 支持哪些 XXL-Job 版本？
A: 理论上支持所有提供 REST API 的 XXL-Job 版本 (>= 2.x)。已在 2.3.x 版本上测试。

---

## 📜 License

[MIT License](LICENSE)

---

## 🙏 致谢

- [XXL-Job](https://github.com/xuxueli/xxl-job) - 优秀的分布式任务调度平台
- [NestJS](https://nestjs.com/) - 渐进式 Node.js 框架
- [Prisma](https://www.prisma.io/) - 下一代 TypeScript ORM
- [Ant Design](https://ant.design/) - 企业级 UI 设计语言

---

## 📧 联系方式

- **Issue Tracker**: [GitHub Issues](https://github.com/your-org/xxljob-enhance/issues)
- **Email**: your-email@example.com

---

<p align="center">
  Made with ❤️ by Your Team
</p>
